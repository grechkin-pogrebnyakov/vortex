...
static TVars **inverse_matrix(TVars **M, size_t size) {
    ...
    for (size_t k = 0; k < size-1; k++) {
        ...
        size_t line = k +1;
        for (size_t i = k + 1; i < size; i++) {               // Ð’Ñ‹Ð±Ð¾Ñ€ Ð³Ð»Ð°Ð²Ð½Ð¾Ð³Ð¾ ÑÐ»ÐµÐ¼ÐµÐ½Ñ‚Ð°
            if (fabs( M[i][k + 1] ) > mx) {
                mx = fabs( M[i][k + 1] );
                line = i;
            }//if
        ...
        err = move_line( M_inv, size, k + 1, line );                      // Ð¿ÐµÑ€ÐµÑÑ‚Ð°Ð½Ð¾Ð²ÐºÐ° ÑÑ‚Ñ€Ð¾Ðº
        PR = POR[k + 1];
        POR[k + 1] = POR[line];
        POR[line] = PR;
        if (err) {
...
int incr_vort_quont(Vortex **p_host, Vortex **p_dev, PVortex **v_host, PVortex **v_dev, TVars **d_dev, size_t *size) {
    ...
    rnd_host = (float*)malloc( sizeof(float) * INCR_STEP );
...
//    std::cout << "first collapse\n";
    for (int cc = 0; cc < NCOL; ++cc) {
        ...
        int *Sety = NULL;
        int *COL = NULL;
        cuerr=cudaMalloc ( (void**)&Setx, *n * sizeof( int ) );
        cuerr=cudaMalloc ( (void**)&Sety, *n * sizeof( int ) );
        cuerr=cudaMalloc ( (void**)&COL, *n * sizeof( int ) );
