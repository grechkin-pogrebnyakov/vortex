...
TVars   **inverse_matrix(TVars **M, size_t size) {
    int err = 0;
    size_t *POR = NULL;                                        // массив дл€ учЄта перестановки строк
    POR = new size_t[size];
    if (!POR) return NULL;
    size_t PR;                                                 // переменна€ дл€ учЄта перестановок строк
    for (size_t i = 0; i < size; i++) {
        POR[i]=i;
    }
    TVars b;
    TVars **M_inv = NULL;                                   // обратна€ матрица
    ...
    for (size_t k = 0; k < size-1; k++) {
        ...
        size_t line=k+1;
        for (size_t i = k+1; i < size; i++) {               // ¬ыбор главного элемента
            if (fabs(M[i][k+1])>mx) {
                mx=fabs(M[i][k+1]);
                line=i;
            }//if
